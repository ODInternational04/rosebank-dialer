...
type CallStatusFilter = 'all' | 'called' | 'not_called' | 'needs_callback'
...
export default function ClientsPage() {
  ...
  const [notCalledCount, setNotCalledCount] = useState(0)
  const [needsCallbackCount, setNeedsCallbackCount] = useState(0)
  ...
  const fetchClients = useCallback(async (isInitialLoad = false) => {
    try {
      ...
      if (response.ok) {
        ...
        setCalledCount(data.calledCount || 0)
        setNotCalledCount(data.notCalledCount || 0)
        setNeedsCallbackCount(data.needsCallbackCount || 0)
      }
    } catch (error) {
  ...
  const handle3CXCallStart = useCallback(async (callSession: CallSession) => {
    console.log('🟣 handle3CXCallStart called with callSession:', callSession)
    try {
      const token = localStorage.getItem('token')
      console.log('🟣 Token retrieved:', token ? 'exists' : 'missing')
      if (token) {
        console.log('🟣 Updating user status to start_call...')
      ...
      console.log('🟣 Looking for client in clients array...')
      const client = clients.find(c => c.id === callSession.clientId)
      console.log('🟣 Client found:', client ? client.principal_key_holder : 'NOT FOUND')
      if (client) {
        console.log('🟣 Setting selectedClient...')
        setSelectedClient(client)
        console.log('🟣 Setting autoStartTimer to true...')
        setAutoStartTimer(true)
        console.log('🟣 Opening call modal...')
        setShowCallModal(true)
        console.log('✅ Call log modal opened for active call to:', client.principal_key_holder)
      } else {
        console.error('❌ Client not found in clients array for ID:', callSession.clientId)
      }
    } catch (error) {
  ...
  // Stats are now provided by the API for all pages, not just current page
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className={`card p-4 cursor-pointer border-2 transition-all ${
            callStatusFilter === 'needs_callback' ? 'border-red-500 bg-red-50' : 'border-gray-200 hover:border-gray-300'
          }`} onClick={() => { setCallStatusFilter('needs_callback'); setCurrentPage(1) }}>
            <div className="flex items-center">
              <ExclamationTriangleIcon className="w-8 h-8 text-red-600" />
              <div className="ml-3">
                <p className="text-sm font-medium text-gray-900">Needs Callback</p>
                <p className="text-2xl font-bold text-red-600">{needsCallbackCount}</p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="needs_callback">⚠️ Needs Callback (Missed/Busy/Declined)</option>
                </select>
              </div>
